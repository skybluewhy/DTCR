...
from utils import proc_data_for_bind
...
class MaskDiffusion(DiscreteDiffusionMatrixBase):
    def __init__(self,
        ...
        self.mask = torch.nn.functional.one_hot(torch.tensor(self.mask_id), num_classes=self.dim).unsqueeze(1).repeat(1, self.dim).float()
        self.device = device
    ...
    def _create_state(self):
        ...
        betas = torch.cat((torch.tensor([0.0], device=self.device), self.schedule(torch.arange(self.num_steps, device=self.device)).to(self.device))).float()
        alphas = 1 - betas
        state = torch.cumprod(alphas, dim=0)
        state[-1] = 0.0
        if self.schedule_name == "blosum":
            Q_t = []  # scheduled matrix
            from get_blosum_prob import q_blosum, cumprod_matrix
            q = q_blosum().to(self.device)
            for i in range(len(betas)):
                q_non_diag = torch.ones((self.dim, self.dim)).to(self.device) * q * betas[i]
                norm_constant = (1 - (q_non_diag).sum(axis=0))
                q_diag = torch.tensor(np.identity(self.dim)).to(self.device) * norm_constant
                R = q_diag + q_non_diag
                Q_t.append(R)
            Q_prod = cumprod_matrix(Q_t)
            self.Q_prod = torch.stack(Q_prod).to(torch.float32).to(self.device)  # cumprod of matrices
            self.Q_t = torch.stack(Q_t).to(torch.float32).to(self.device)  # scheduled matrix
        elif self.schedule_name == "random":
            Q_t = []  # scheduled matrix
            from get_blosum_prob import q_blosum, cumprod_matrix
            for i in range(len(betas)):
                q_non_diag = torch.ones((self.dim, self.dim)).to(self.device) / self.dim * betas[i]
                norm_constant = (1 - (q_non_diag).sum(axis=0))
                q_diag = torch.tensor(np.identity(self.dim)).to(self.device) * norm_constant
                R = q_diag + q_non_diag
                Q_t.append(R)
    ...
    def noise_fn(self, q0, t, word_freq_logits):
        if self.schedule_name == "blosum" or self.schedule_name == "random":
            p = self.Q_prod[t]
            prob_at_time_t = q0.float() @ p
            return prob_at_time_t

        p = self.state[t]

        p = torch.clip(p, 0., .999)
        non_mask_prob = p * q0

        mask_prob = 1 - non_mask_prob.sum(-1, keepdims=True) + non_mask_prob[
            ..., self.mask_id].unsqueeze(-1)

        prob_at_time_t = torch.cat((
            non_mask_prob[..., :self.mask_id], mask_prob,
            non_mask_prob[..., self.mask_id + 1:]
        ), dim=-1)
        return prob_at_time_t
    ...
    def sample_stationary(self, size):
        return torch.full(size=size, fill_value=self.mask_id, device=self.device)
    ...
    def qt_reverse(self,
        ...
        if self.schedule_name == "blosum" or self.schedule_name == "random":
            # prob_at_time_t = self.Q_t[t].T @ qt_plus_1
            prob_at_time_t = qt_plus_1.float() @ (self.Q_t[t].squeeze(0)).T
            if return_logits:
                return torch.log(prob_at_time_t + epsilon)
            else:
                return prob_at_time_t
        beta = self.schedule(t)
        qtpls1_at_mask = qt_plus_1[Ellipsis, self.mask_id: self.mask_id + 1]
        non_mask_prob0 = (1 - beta) * qt_plus_1[Ellipsis, :self.mask_id] + beta * qtpls1_at_mask
        non_mask_prob1 = (1 - beta) * qt_plus_1[Ellipsis, self.mask_id + 1:] + beta * qtpls1_at_mask
...
# diffusion_condition 如何使用mask_id
# blosum62以及vocab对应关系
# 前向加噪多个scheduler整合
# 反向去噪加入进化（对于单个采样多个，然后取平均，作为x0）
...
def create_discrete_diffusion_schedule(
    ...
    if kind == "mutual":
        ...
        def schedule_fn(step):
            ...
            betas = torch.linspace(0, max_n, num_steps)
            betas = torch.exp(betas) * (end - start) + start
            betas = betas / betas.max() + 1 / num_steps
...
def p_forward_sample(
    ...
    # In this study, target mask and attention mask are fixed, but they can change during the sampling step
    # by changing the codes in denoise_fn
    logits, new_attention_mask, new_target_mask = denoise_fn(corrupted_input_ids=x_t, timestep=t, attention_mask=attention_mask, target_mask=target_mask)
    probs = torch.nn.Softmax(dim=-1)(logits)
    ...
    # mask = ((t == step_size) & special_case_x0).long()
    # retval = retval_x0 + (1 - mask) * retval
    # retval = torch.cat((retval_x0, pad_t), dim=-1) if t == step_size else retval
    retval = retval_x0 if t == step_size else retval

    if return_x0:
        return retval, retval_x0, new_attention_mask, new_target_mask
    else:
        return retval
...
def generate_data(
    ...
    def sampling_step(step, state, attention_mask, target_mask):
        del step

        t = state.t  # initially, num_steps, and decreases from there.

        ori_logits, x0_logits, new_attention_mask, new_target_mask = p_forward_sample(
            denoise_fn,
            x_t=state.x,
            t=t,
            diffusion=diffusion,
        ...
        logits = ori_logits / temperature

        adjusted_prob = torch.softmax(torch.log(logits + 1e-10) / 0.3, dim=-1)

        # Sample each sequence position independently
        samples = torch.multinomial(adjusted_prob.view(-1, logits.size(-1)), num_samples=1)

        # Recover the original batch and sequence dimensions
        sample = samples.view(logits.size(0), logits.size(1))
        # sample = torch.distributions.categorical.Categorical(probs=logits).sample()
        sample = torch.where(new_target_mask.bool(), sample, input_ids)

        if show_process:
            print(tk.batch_decode(x0, clean_up_tokenization_spaces=False))

        return SamplingState(x=sample, x0=x0, t=t - step_size), new_attention_mask, new_target_mask

    # x = torch.where(target_mask.bool(), mask_token_id, input_ids)
    x = sample_input_ids

    if predict_x0:
        init_state = SamplingState(x, x, torch.tensor([num_steps], device=sample_input_ids.device))
    else:
        init_state = SamplingState(x, None, torch.tensor([num_steps], device=sample_input_ids.device))

    total_steps = num_steps // step_size

    final_state = utils.fori_loop(0, total_steps, sampling_step, init_state, attention_mask, target_mask)
