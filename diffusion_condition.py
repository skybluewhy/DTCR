...
def p_forward_sample(
    ...
    logits, new_attention_mask, new_target_mask, new_input_ids = denoise_fn(corrupted_input_ids=x_t, timestep=t, attention_mask=attention_mask, target_mask=target_mask)
    probs = torch.nn.Softmax(dim=-1)(logits)

    if not predict_x0:
        retval = logits if return_logits else probs
        if return_x0:
            return retval, None, new_attention_mask, new_target_mask, new_input_ids
        else:
            return retval
    ...
    if return_x0:
        return retval, retval_x0, new_attention_mask, new_target_mask, new_input_ids
    else:
        return retval
...
def generate_data(
    ...
    def sampling_step(step, state, attention_mask, target_mask):
        ...
        ori_logits, x0_logits, new_attention_mask, new_target_mask, new_input_ids = p_forward_sample(
            denoise_fn,
            x_t=state.x,
            t=t,
            diffusion=diffusion,
        ...
        logits[:, 45:, :] = top_k_top_p_filtering(logits[:, 45:, :], top_k=topk, top_p=topp)

        samples = torch.distributions.categorical.Categorical(logits=logits).sample()

        sample = samples.view(logits.size(0), logits.size(1))

        sample = torch.where(new_target_mask.bool(), sample, new_input_ids)
    ...
    final_state = utils.fori_loop_reinforce_learn(0, total_steps, sampling_step, init_state, attention_mask, target_mask)
